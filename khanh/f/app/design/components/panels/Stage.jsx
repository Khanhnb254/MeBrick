...
export default function Stage({
  ...
  useEffect(() => {
    ...
    const compute = () => {
      // Trên mobile (<768px): dùng window.innerWidth để đo đúng full screen width
      // Trên desktop: đo từ element để tránh tràn layout
      const isMobile = window.innerWidth < 768;
      const containerW = isMobile
        ? window.innerWidth
        : (stageWorkspaceRef.current?.getBoundingClientRect().width ?? window.innerWidth);
      const available = containerW - 16; // trừ padding photo-frame-studio
      setCanvasScale(available > 0 && available < canvasSize.width ? available / canvasSize.width : 1);
    };
