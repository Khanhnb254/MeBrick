...
export function useLegoCharacter({
  ...
  // Pick desktop or mobile config based on actual screen width
  const isMobile = typeof window !== "undefined" && window.innerWidth < 1024;
  const partConfig = isMobile
    ? LEGO_CONFIG.partConfigMobile
    : LEGO_CONFIG.partConfig;
  const assemblyConfig = isMobile
    ? LEGO_CONFIG.assemblyConfigMobile
    : LEGO_CONFIG.assemblyConfig;
        ...
        setStickers((prevStickers) =>
          prevStickers.map((sticker) => {
            ...
            if (sticker.layerType === "face") {
              ...
              return { ...sticker, x: pos.x, y: pos.y + offsetYExtra };
            }
